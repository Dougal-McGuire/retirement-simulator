...
export function formatPercent(value: number, decimals: number = 1): string {
  ...
  const validDecimals = typeof decimals === 'number' && !isNaN(decimals) ? Math.max(0, Math.min(20, decimals)) : 1;
...
export function getSpendingPercent(amount: number, spending: any): number {
...
export function getTotalMonthlySpending(spending: any): number {
...
export function getRetirementMedian(projections: any, person: any): number {
  const retirementMilestone = projections.milestones.find(
    (m: any) => m.age === person.retireAge
  );
  return retirementMilestone ? retirementMilestone.p50 : 0;
}

export function getRetirement10th(projections: any, person: any): number {
  const retirementMilestone = projections.milestones.find(
    (m: any) => m.age === person.retireAge
  );
...
export function getRetirement90th(projections: any, person: any): number {
  const retirementMilestone = projections.milestones.find(
    (m: any) => m.age === person.retireAge
  );
...
export function getFinalMedian(projections: any): number {
  const lastMilestone = projections.milestones[projections.milestones.length - 1];
  return lastMilestone ? lastMilestone.p50 : 0;
}

export function getFinal10th(projections: any): number {
  const lastMilestone = projections.milestones[projections.milestones.length - 1];
  return lastMilestone ? lastMilestone.p10 : 0;
}

export function getFinal90th(projections: any): number {
  const lastMilestone = projections.milestones[projections.milestones.length - 1];
