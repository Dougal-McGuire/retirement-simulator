...
export async function renderLineChart(
  ...
  type LineRow = { x: number; y: number; series: string }
  const data: LineRow[] = []
  series.forEach((s) => {
  ...
  const lineLayer: any = {
    ...
    encoding: {
      ...
      strokeWidth: {
        condition: [{ test: "datum.series === 'P50 (Median)'", value: 3 }],
        value: 1.5,
      },
    },
  }
...
export async function renderBarChart(
  ...
  const { width = 560, height = 320, xTitle = 'Category', yTitle = 'Value', title = '' } = options
...
export async function renderStackedAreaChart(
  ...
  const { width = 560, height = 320, xTitle = 'X Axis', yTitle = 'Y Axis', title = '' } = options

  type StackedRow = { x: number; y: number; category: string }
  const data: StackedRow[] = []
  series.forEach((s) => {
    s.values.forEach((point) => {
      data.push({
        x: point.x,
