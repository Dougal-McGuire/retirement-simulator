...
export default function SetupPage() {
  ...
  const glassCardClass = 'border-2 border-neo-black bg-card shadow-neo'
          ...
          <div className="space-y-6">
            ...
            <div>
              <Label
                ...
                className="font-mono text-[0.68rem] font-semibold uppercase tracking-[0.12em] text-neo-black"
              >
                {t('personal.fields.retirementAge.label')}
              </Label>
              <div className="mt-3 border-2 border-neo-black bg-card px-4 py-3 shadow-neo-sm">
                <Slider
                  id="retirementAge"
      ...
      case 'assets':
        return (
          <div className="space-y-6">
            ...
            <div className="space-y-4 border-2 border-neo-black bg-card px-4 py-5 shadow-neo-sm">
              <div>
                <h5 className="text-[0.72rem] font-extrabold uppercase tracking-[0.16em] text-neo-black">
                  {t('assets.oneTimeIncomes.title')}
                </h5>
                <p className="mt-1 text-[0.72rem] font-medium leading-5 tracking-normal text-muted-foreground">
  ...
  return (
    <div className="relative min-h-screen pb-16 text-foreground">

      <header id="navigation" className="relative z-10 pt-12 pb-10">
        <div className="mx-auto max-w-[90rem] px-2 sm:px-3 lg:px-4">
          <div className={cn(glassCardClass, 'relative overflow-hidden px-6 py-8 sm:px-8 lg:px-10')}>
            <div className="relative flex flex-col gap-10">
              <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                <div className="flex flex-col gap-5 text-neo-black">
                  <div className="flex flex-wrap items-center gap-3 font-mono text-[0.68rem] font-semibold uppercase tracking-[0.2em]">
                    <span className="inline-flex items-center gap-2 border-2 border-neo-black bg-neo-yellow px-4 py-1.5 text-neo-black shadow-neo-sm">
                      {t('header.badges.guide')}
                    </span>
                    <span className="inline-flex items-center gap-2 border-2 border-neo-black bg-card px-4 py-1.5 text-muted-foreground shadow-neo-sm">
                      {t('header.badges.time')}
                    </span>
                  </div>
                  <div>
                    <h1 className="text-3xl font-black tracking-[0.08em] sm:text-4xl">
                      {t('header.title')}
                    </h1>
                    <p className="mt-4 max-w-2xl text-sm font-medium leading-7 text-foreground/75">
                      {t('header.subtitle')}
                    </p>
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-4 border-t-2 border-neo-black pt-6 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex flex-wrap items-center gap-3 font-mono text-[0.72rem] font-semibold uppercase tracking-[0.12em]">
                  <span className="border-2 border-neo-black bg-card px-4 py-1 shadow-neo-sm">
                    {progressLabel}
                  </span>
                ...
                <div className="relative h-5 w-full overflow-hidden border-2 border-neo-black bg-card shadow-neo-sm sm:max-w-sm">
                  <div
                    className="relative h-full overflow-hidden bg-neo-green transition-all duration-300 ease-out"
                  ...
                  >
                    <div
                      ...
                      style={{
                        backgroundImage:
                          'repeating-linear-gradient(45deg, transparent, transparent 10px, rgba(5,8,15,0.16) 10px, rgba(5,8,15,0.16) 20px)',
                      }}
                    />
                  </div>
                </div>
      ...
      >
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[360px_minmax(0,1fr)] xl:grid-cols-[380px_minmax(0,1fr)]">
          <aside className={cn(glassCardClass, 'space-y-6 p-5 lg:p-6')}>
            <div className="space-y-3">
              ...
              {/* Time estimate */}
              <div className="border-2 border-dashed border-neo-black bg-neo-blue/10 px-3 py-2">
                ...
                return (
                  <div key={step.id} className="relative pl-12">
                    {index < steps.length - 1 && (
                      <span
                        className={cn(
                          'absolute left-5 top-11 h-[calc(100%-2.75rem)] w-0.5',
                    ...
                    <button
                      ...
                      className={cn(
                        'absolute left-0 top-0 flex h-10 w-10 items-center justify-center border-2 border-neo-black bg-card font-extrabold text-neo-black shadow-neo-sm transition-neo',
                        isCompleted && 'bg-neo-green text-neo-black',
                        isActive && !isCompleted && 'bg-neo-black text-neo-white ring-2 ring-neo-blue ring-offset-2',
                    ...
                    <div>
                      <p
                        className={cn(
                          'font-mono text-[0.76rem] font-bold uppercase tracking-[0.12em]',
          ...
          <section className="space-y-0">
            <Card className={cn(glassCardClass)}>
              <CardHeader className="border-b-2 border-neo-black bg-[linear-gradient(135deg,hsl(var(--card)),rgba(42,213,118,0.08))]">
                <CardTitle className="text-xl font-extrabold uppercase tracking-[0.1em] text-neo-black">
                  {steps[currentStep].title}
                </CardTitle>
                <CardDescription className="mt-2 max-w-2xl text-sm font-medium leading-6 text-muted-foreground">
            ...
            <div className="mt-4 flex flex-col gap-3 border-2 border-neo-black bg-card p-4 shadow-neo-sm sm:flex-row sm:items-center sm:justify-between">
              <Button
                variant="outline"
                size="sm"
                onClick={handleBack}
